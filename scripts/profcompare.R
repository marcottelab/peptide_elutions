    ...
    ## matrix
      ...
      for (i in seq_len(ncx)) {
        for (j in seq_len(i)) {
          ...
          #print(x2)
          #print(y2)
          # If both are present, or both absent
          sa <- sum((x2 > 0 & y2 > 0)) #  | (x2 ==0 & y2==0)) # same

          # If one present, and one absent
          di <- sum((x2 > 0 & y2 == 0) | (x2 == 0 & y2 > 0)) # diff
 
          #r[i, j] <-  if(any(ok)) sum(abs(x2 - y2), na.rm = TRUE) else NA
          r[i, j] <-  if(any(ok)) sa/sum(sa + di) else NA
         # print(sa/sum(sa + di))
        }
